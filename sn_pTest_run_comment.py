...
# Random combination
def product_squence(rule_use_all):
    sum_count = 1
    rule_use = []
...
class SNP_System():
    ...
    def runSimulationStep(self,system,filename = None):
        ...
        sum_count,rule_use_index = product_squence(use_rule_all)
        ...
        with open('result_%s.txt'%filename, 'a+') as f:
            f.write(form_trans)
        ...
        while (step >= 1):
            ...
            if sum_counts_sub <= 0 or step == 0:
                try:
                    from graphviz import Digraph
                    dot.view()
                    dot.render('test-output_pdf/round-table.gv', view=False)
                except:
                    print('not found moudle graphviz')
                break
...
if __name__ == "__main__":
    filename = 'ex_1.txt'
    if os.path.exists('result_%s.txt'%filename):
        os.remove('result_%s.txt'%filename)
    system = readInputFile(filename)
    record_out_step,SNP3=system.runSimulation(system,5,filename)
    print(len(SNP3) ,record_out_step)
